  ...
  useEffect(() => {
    ...
    getChannelProfile(username)
      .then((res) => {
        ...
        setChannel(ch);
        setSubscribed(ch?.is_subscribed || false);
...
const handleSubscribe = async () => {
  try {
    ...
    setSubscribed((prev) => {
      setSubCount((count) => (prev ? count - 1 : count + 1));
      return !prev;
    });
  } catch {
        ...
        <div className="flex flex-1 flex-col sm:flex-row sm:items-end justify-between gap-4">
          <div>
            ...
            <div className="flex flex-wrap gap-5 mt-3 text-sm text-gray-400">
              ...
              <span><span className="text-white font-semibold">{formatCount(channel.subscribedTo)}</span> subscriptions</span>
